...
export default async function suite() {
  describe('Vesting Contract', async () => {
    ...
    beforeEach(async function () {
      ...
      this.advanceTime = async (seconds: number) => {
        // @ts-ignore
        await web3.currentProvider.send({method: "evm_increaseTime", params: [seconds], id: 0}, function() {});
        // @ts-ignore
    ...
    it('Correct vesting stats', async function () {
      let investorID = await this.vestingContract.getInvestorID(this.investor1);
      let investorInfo = await this.vestingContract.getInvestor(investorID);
      expect(investorInfo.cliff).to.be.equal((1*30*24*60*60).toString());
    ...
    it('Cannot claim before start timestamp', async function () {
      ...
      let totalClaimed =  await this.vestingContract.getTotalClaimed(investorID);
    ...
    it('Can claim at TGE', async function () {
      let investorID = await this.vestingContract.getInvestorID(this.investor2);
      let cliff = parseInt(await this.vestingContract.getCliff(investorID));
      await this.advanceTime(cliff); //After start time but not before end of cliff
      const start = parseInt(await this.vestingContract.getStart(investorID));
      // @ts-ignore
      ...
      */
      ...
      let totalClaimed = parseInt(fromWei(await this.vestingContract.getTotalClaimed(investorID)));
      // @ts-ignore
      expect(totalClaimed).to.be.equal(initialUnlock);

      let balanceOfInvestor = parseInt(fromWei(await this.tcpToken.balanceOf(this.investor2)));
      // @ts-ignore
    ...
    it('Can claim 1 month', async function () {
      ...
      let totalClaimed = parseInt(fromWei(await this.vestingContract.getTotalClaimed(investorID)));
      // @ts-ignore
      expect(totalClaimed).to.be.equal((totalAmount/numberOfMonths) + initialUnlock); //TGE + 1 month
    ...
    it('Can claim 3 months at one time', async function () {
      ...
      let totalClaimed = parseInt(fromWei(await this.vestingContract.getTotalClaimed(investorID)));
      // @ts-ignore
      expect(totalClaimed).to.be.equal(((totalAmount/numberOfMonths)*4) + initialUnlock); //TGE + 4 months

      let balanceOfInvestor = parseInt(fromWei(await this.tcpToken.balanceOf(this.investor2)));
      // @ts-ignore
      expect(balanceOfInvestor).to.be.equal(((totalAmount/numberOfMonths)*4) + initialUnlock); //TGE + 4 months
    });
    ...
    it('Can claim the last payment', async function () {
      ...
      let totalClaimed = parseInt(fromWei(await this.vestingContract.getTotalClaimed(investorID)));
      // @ts-ignore
      expect(totalClaimed).to.be.equal(((totalAmount/numberOfMonths)*9) + initialUnlock); //TGE + 9 months

      let balanceOfInvestor = parseInt(fromWei(await this.tcpToken.balanceOf(this.investor2)));
      // @ts-ignore
      expect(balanceOfInvestor).to.be.equal(((totalAmount/numberOfMonths)*9) + initialUnlock); //TGE + 9 months
    });
