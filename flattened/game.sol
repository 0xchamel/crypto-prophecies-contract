...
contract CryptoPropheciesGame is ReentrancyGuard, Ownable {
    ...
    struct Battle {
        ...
        uint256 challengerProphetId;
        uint256 chargerProphetId;
    }
    ...
    event SetWinner(uint256 indexed battleId, address indexed winner);
    ...
    event KingdomFeeDeducted(uint256 indexed amount);
    ...
    event BattleCreated(
        ...
        uint256 challengerProphetId,
        uint256 chargerProphetId,
        uint256 challengerProphetTier,
        uint256 chargerProphetTier
    );

    event DailyPrizeTicketAdded(address indexed account, uint256 amount);

    constructor(address _MPOT, address _TCP) {
        MPOT = IMPOT(_MPOT);
        TCP = _TCP;
    }
    ...
    function createBattle(
        address _challenger,
        uint256 _challengerProphetId,
        address _charger,
        uint256 _chargerProphetId,
        ...
        battles[battleId] = Battle(
            ...
            2 * _wagerAmount,
            block.timestamp,
            _challengerProphetId,
            _chargerProphetId
        );

        (, uint16 challengerRarity, , , , ) = IProphet(prophetAddr).prophets(_challengerProphetId);
        (, uint16 chargerRarity, , , , ) = IProphet(prophetAddr).prophets(_chargerProphetId);
    ...
    function _transferFundsToWinner(uint256 _battleId) internal {
        require(kfBurnAddr != address(0), "kfBurnAddr not set");
        require(kfCustodyAddr != address(0), "kfCustodyAddr not set");
        require(kfDailyPrizeAddr != address(0), "kfDailyPrizeAddress not set");
        ...
        IERC20(TCP).safeTransfer(kfCustodyAddr, kingdomFee / 10);
